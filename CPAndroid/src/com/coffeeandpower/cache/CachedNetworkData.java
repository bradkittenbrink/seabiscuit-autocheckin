...
public class CachedNetworkData extends Observable {
    ...
    private Location userLocationWhenDataCollected = new Location(
            "userDataLocation");
    ...
    public void setNewData(DataHolder newData, double[] userLocation) {
        ...
        if (newData.getResponseMessage().equals("HTTP 200 OK")) {
            ...
            // Log.d("CachedNetworkData","Setting user location to: " +
            ...
            if (Constants.debugLog)
                Log.d("CachedNetworkData",
                        "Sending notifyObservers with received data from API call: "
                                + type + "...");
            ...
            if (Constants.debugLog)
                Log.d("CachedNetworkData",
                        "Skipping notifyObservers for API call: " + type);
    ...
    public void sendCachedData() {
        if (hasData) {
            if (Constants.debugLog)
                Log.d("CachedNetworkData", "Sending cached data for API: "
                        + this.type + "...");
