...
public class HttpUtil
{
			...
			if (responseString != null)
			{
				...
				if (json != null)
				{
					...
					if (payload != null)
					{
						...
						// Check in history, I took only first TWO results, but
						// maybe we need more than two....
								...
								for (int x = 0; x < reviewsArray.length(); x++)
								{
									...
									if (objReviewFromArray != null)
									{
										...
										reviews.add(new Review(objReviewFromArray.optInt("id"), objReviewFromArray
												.optString("author"), objReviewFromArray.optString("title"),
												objReviewFromArray.optString("type"), objReviewFromArray
														.optString("create_time"), objReviewFromArray
														.optString("skill"), objReviewFromArray.optString("rating"),
												objReviewFromArray.optString("is_love"), objReviewFromArray
														.optString("tip_amount"), objReviewFromArray
														.optString("review"), objReviewFromArray
														.optString("ratingImage"), objReviewFromArray
						...
						if (arrayEdu != null)
						{
							...
							for (int x = 0; x < arrayEdu.length(); x++)
							{
								...
								if (objEdu != null)
								{
									education.add(new Education(objEdu.optString("school"), objEdu.optInt("startDate"),
											objEdu.optInt("endDate"), objEdu.optString("concentrations"), objEdu
													.optString("degree")));
								}
							}
						}
						...
						// [0] Object UserResume, [1] array list favourite
						// checkin locatios
						ArrayList<Object> tempHolder = new ArrayList<Object>();
						tempHolder
								.add(new UserResume(nickName, statusText, urlPhoto, urlPhotoLarge, joined, bio,
										totalEarned, totalTipsEarned, totalMissionCountAsRecipient, distinctTipPayers,
										totalSpent, totalTipsSpent, totalMissionCountAsPayer, distinctTipRecipients,
										totalEarnedFromMe, totalMissionsFromMe, totalMissionsAsAgent,
										totalMissionsAsClient, totalMissions, totalFunded, skillSet, hourlyBillingRate,
										verifiedLinkedIn, linkedInProfileLink, verifiedFacebook, facebookProfileLink,
										verifiedMobile, trusted, jobTitle, checkInId, userId, lat, lng, checkInDate,
										checkIn, checkOutDate, checkOut, foursquare, foursquareId, venueName,
										venueAddress, city, state, zip, phone, icon, visible, photoUrlUnUsed,
										formattedPhone, usersHere, reviewsPage, reviewsTotal, reviewsRecords,
										reviewsLoveReceived, reviews, education, work, locationLat, locationLng));
	...
	public DataHolder uploadUserProfilePhoto()
	{
		...
		File file = new File(Environment.getExternalStorageDirectory() + ActivitySettings.IMAGE_FOLDER,
				"photo_profile.jpg");
							...
							for (int x = 0; x < arrayChat.length(); x++)
							{
								...
								if (objMess != null)
								{
									messages.add(new ChatMessage(objMess.optInt("id"), objMess.optInt("user_id"),
											objMess.optString("entry_text"), objMess.optString("nickname"), objMess
													.optString("date"), objMess.optString("photo_url"), objMess
	...
	public DataHolder getVenuesAndUsersWithCheckinsInBoundsDuringInterval(double data[], int numberOfDays)
	{
		...
		HttpGet get = new HttpGet(AppCAP.URL_WEB_SERVICE + AppCAP.URL_API
				+ "?action=getVenuesAndUsersWithCheckinsInBoundsDuringInterval" + "&sw_lat=" + data[0] + "&sw_lng="
				+ data[1] + "&ne_lat=" + data[2] + "&ne_lng=" + data[3] + "&checked_in_since="
				+ (System.currentTimeMillis() / 1000 - (86400 * numberOfDays)) + userLat + userLng + "&version=0.1");
		// Log.d("LOG", "time interval: " + (System.currentTimeMillis() /1000 -
		// (86400 * numberOfDays)));
		try
		{
							...
							for (int x = 0; x < arrayVenues.length(); x++)
							{
								...
								if (objVenue != null)
								{
											...
											for (int y = 0; y < userIds.length(); y++)
											{
												...
												if (o != null)
												{
													...
													arrayCheckins.add(new CheckinData(userId,
															o.optInt("checkin_count"), o.optInt("checked_in")));
												}
											}
										}
									}
									...
									venues.add(new VenueSmart(objVenue.optString("name"),
											objVenue.optString("address"), objVenue.optString("city"), objVenue
													.optString("state"), objVenue.optString("distance"), objVenue
													.optString("foursquare"), objVenue.optInt("checkins"), objVenue
													.optInt("checkins_for_week"), objVenue
													.optInt("checkins_for_interval"), objVenue.optString("photo_url"),
											objVenue.optString("phone"), objVenue.optString("formatted_phone"),
											objVenue.optDouble("lat"), objVenue.optDouble("lng"), arrayCheckins));
								}
							}
						}
							...
							for (int x = 0; x < arrayUsers.length(); x++)
							{
								...
								if (objUser != null)
								{
									...
									UserSmart singleUserMap = new UserSmart(
											...
											objUser.optInt("checked_in"), objUser.optString("foursquare"),
											objUser.optString("venue_name"), objUser.optInt("checkin_count"),
											objUser.optString("skills"), objUser.optBoolean("met"));

									// if herenow exist, than that value will be
									// used
			...
			if (responseString != null)
			{
				...
				if (json != null)
				{
					...
					if (objPayload != null)
					{
						...
						int count = objPayload.optInt("count"); // I will use
																// arratUsers.size()
																// instead of
																// count
							...
							if (venues != null)
							{
								...
								for (int m = 0; m < venues.length(); m++)
								{
									...
									if (venue != null)
									{
										...
										venuesArray.add(new Venue(id, name, address, crossStreet, lat, lng, distance,
												postalCode, city, state, country, categoryName, categoryPluralName,
												categoryShortName, checkinsCount, usersCount, tipCount, hereNowCount,
												"", ""));
									}
								}
			...
			if (responseString != null)
			{
				...
				if (json != null)
				{
					...
					if (objPayload != null)
					{
						...
						result.setObject(new UserTransaction(objPayload.optInt("userid"), objPayload
								.optString("nickname"), objPayload.optString("username"), objPayload
								.optString("status_text"), objPayload.optString("status"), objPayload
								.optString("active"), objPayload.optString("photo"), objPayload
								.optString("photo_large"), objPayload.optDouble("lat"), objPayload.optDouble("lng"),
			...
			if (responseString != null)
			{
				...
				if (json != null)
				{
					...
					result.setObject(new User(userId, favoriteEnabled, favoriteCount, myFavoriteCount, moneyReceived,
							offersPaid, balance, nickName, userName, statusText, status, active, photo, photoLarge,
							lat, lng));
					result.setResponseCode(AppCAP.HTTP_REQUEST_SUCCEEDED);
					return result;
				}
			...
			if (responseString != null)
			{
				...
				if (json != null)
				{
					...
					if (objPayload != null)
					{
						...
						result.setObject(new Object[]
						{ objPayload.optString("push_distance"), objPayload.optString("checked_in_only") });
					}
				}
			}
