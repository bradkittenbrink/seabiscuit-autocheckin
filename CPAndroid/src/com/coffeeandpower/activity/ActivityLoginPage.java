...
public class ActivityLoginPage extends RootActivity
{
	...
	private void connectLinkedIn()
	{
		...
		lastAuthorize.initialize((String) getResources().getText(R.string.linkedInApiKey), (String) getResources()
				.getText(R.string.linkedInApiSec));
		if (AppCAP.getUserLinkedInID().equals(""))
		{
	...
	public void onNewIntent(Intent intent)
	{
		...
		if (lastAuthorize == null) return;

		if (lastAuthorize.verify(verifier))
		{
			new Thread(new OAuthSignUpAction(lastAuthorize, // the service
															// provider
	...
	public class LoginAction extends ActivityUtils.Action
	{
		...
		public void run()
		{
			...
			{
				handler.sendEmptyMessage(result.getResponseCode());
				if (result.getResponseCode() == AppCAP.ERROR_SUCCEEDED_SHOW_MESS) service.clearSettings(); // assume
																											// that
																											// the
																											// tokens
																											// have
