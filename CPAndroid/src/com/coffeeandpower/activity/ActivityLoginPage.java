    	...
    	if (lastAuthorize.verify(verifier))
    	{
    		new Thread( 
				new OAuthSignUpAction(
					lastAuthorize, //the service provider
					new ActivityUtils.JoinProgressHandler(this), //the progress handler for this action
					new LoginAction(
							lastAuthorize,
							new ActivityUtils.LoginProgressHandler(this), 
							null)	
    ...
    {		
		...
		public void run() {
			...
			if (result!=null){
				...
				if (result.getResponseCode() == AppCAP.HTTP_REQUEST_SUCCEEDED &&
					action != null)
    	...
    	public void run() {
			...
			if (result!=null){
				...
				if (result.getResponseCode() == AppCAP.ERROR_SUCCEEDED_SHOW_MESS)
					service.clearSettings();  //assume that the tokens have expired... 
			} else {
				handler.sendEmptyMessage(AppCAP.HTTP_ERROR);
			}			    		
