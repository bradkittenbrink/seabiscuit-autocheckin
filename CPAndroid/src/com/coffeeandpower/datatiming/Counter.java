...
public class Counter {
    ...
    private CachedNetworkData venuesWithCheckinsCache = new CachedNetworkData(
            "venuesWithCheckins");
    private CachedNetworkData nearbyVenuesCache = new CachedNetworkData(
            "nearbyVenues");
    ...
    public void getCachedDataForAPICall(String apicall, Observer context) {
        ...
        if (apicall.equals("venuesWithCheckins")) {
            if (Constants.debugLog)
                Log.d("Counter", "Enabling venuesWithCheckins API for "
                        + context.toString());
    ...
    public void getCachedDataForAPICalls(String apicall1, String apicall2,
            Observer context) {
    ...
    public void stoppedObservingAPICall(String apicall, Observer context) {
        if (apicall.equals("venuesWithCheckins")) {
            if (Constants.debugLog)
                Log.d("Counter", "Removing venuesWithCheckins observer for "
                        + context.toString() + ".");
            ...
            if (Constants.debugLog)
                Log.d("Counter", "Removing nearbyVenues observer for "
                        + context.toString() + ".");
            ...
            if (Constants.debugLog)
                Log.d("Counter", "Removing contactsList observer for "
                        + context.toString() + ".");
    ...
    public void stop() {
        ...
        if (isRunning == true) {
            ...
            if (Constants.debugLog)
                Log.d("Counter",
                        "Warning: Tried to stop Counter but it was not started...");
        }
    }
    ...
    public void start() {
        ...
        if (isRunning == false) {
            ...
            if (Constants.debugLog)
                Log.d("Counter",
                        "Warning: Tried to start Counter when it was already running...");
        }
        ...
        public void run() {
            ...
            new Thread(new Runnable() {
                public void run() {
                    ...
                    // isFirstRun = true;
                    if (AppCAP.getMapCenterLatLon()[0] == 0
                            && AppCAP.getMapCenterLatLon()[1] == 0) {
                    ...
                    if (!skipAPICallsThisRun) {
                        ...
                        if (Constants.debugLog)
                            Log.d("Counter", "API calls: Using coordinates: "
                                    + latForAPI + ", " + lonForAPI);
                        if (Constants.debugLog)
                            Log.d("Counter",
                                    "Cache Status: venuesWithCheckins: "
                        ...
                        if (venuesWithCheckinsCache.isActive()
                            ...
                            if (allowCachedDataThisRun
                                ...
                                if (Constants.debugLog)
                                    Log.d("Counter",
                                            "Sending cached data for venuesWithCheckins");
                                venuesWithCheckinsCache.sendCachedData();
                                cachedDataSentThisUpdate = true;
                            } else {
                                if (Constants.debugLog)
                                    Log.d("Counter",
                                            "Refreshing venuesWithCheckinsCache...");
                                venuesWithCheckinsCache
                                        .setNewData(AppCAP
                                                .getConnection()
                                ...
                                if (Constants.debugLog)
                                    Log.d("Counter",
                                            "Called VenuesWithCheckins, Received: "
                                                    + venuesWithCheckinsCache
                                                            .getData()
                                                            .getResponseMessage());
                        ...
                        if (nearbyVenuesCache.isActive()
                            ...
                            if (allowCachedDataThisRun
                                ...
                                if (Constants.debugLog)
                                    Log.d("Counter",
                                            "Sending cached data for nearbyVenues");
                        ...
                        if (contactsListCache.isActive()
                            ...
                            if (allowCachedDataThisRun
                                ...
                                if (Constants.debugLog)
                                    Log.d("Counter",
                                            "Sending cached data for contactsList");
                                contactsListCache.sendCachedData();
                                cachedDataSentThisUpdate = true;
                            } else {
                                if (Constants.debugLog)
                                    Log.d("Counter",
                                            "Refreshing contactsListCache...");
                                contactsListCache.setNewData(AppCAP
                                        .getConnection().getContactsList());
                                if (Constants.debugLog)
                                    Log.d("Counter",
                                            "Called getContactsList, Received: "
                                                    + contactsListCache
                                                            .getData()
                                                            .getResponseMessage());
                                apisCalledThisUpdate = true;
                        ...
                        if (cachedDataSentThisUpdate) {
                            if (Constants.debugLog)
                                Log.d("Counter",
                                        "Cached Data sent this update.");
                        }
                        if (apisCalledThisUpdate) {
                            if (Constants.debugLog)
                                Log.d("Counter",
                                        "New API calls this made this update.");
                        }
                        if (!cachedDataSentThisUpdate && !apisCalledThisUpdate)
                            if (Constants.debugLog)
                                Log.d("Counter", "No data consumers active.");

                        // Clear any flags that control behavior for a single
                        // update
                    ...
                    if (numberOfCalls < 20) {
                        ...
                        if (Constants.debugLog)
                            Log.d("Counter",
                                    "Turning off counter until user activity");
